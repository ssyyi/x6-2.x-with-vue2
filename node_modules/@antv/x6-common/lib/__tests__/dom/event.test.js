...
const core_1 = require("../../dom/event/core");
    ...
    describe('events', () => {
      ...
      `;
        class EventDom {
            ...
            on(p1, p2, p3, p4) {
                dom_1.Dom.Event.on(this.node, p1, p2, p3, p4);
                return this;
            }
            once(p1, p2, p3, p4) {
                dom_1.Dom.Event.once(this.node, p1, p2, p3, p4);
                return this;
            }
            off(p1, p2, p3) {
                dom_1.Dom.Event.off(this.node, p1, p2, p3);
                return this;
            }
            trigger(p1, p2, p3) {
                dom_1.Dom.Event.trigger(this.node, p1, p2, p3);
                return this;
            }
            findOne(selector) {
                const one = this.node.querySelector(selector);
        ...
        describe('on()', () => {
            ...
            it('should not bind event on invalid target', () => {
                ...
                const spy = sinon_1.default.spy();
                dom_1.Dom.Event.on(text, 'click', spy);
                dom_1.Dom.Event.trigger(text, 'click');
            ...
            it('should ignore invalid handler', () => {
                const div = new EventDom();
                expect(() => div.on('click', null)).not.toThrow();
            });
        ...
        describe('trigger()', () => {
            ...
            it('should also trigger inline binded event', () => {
                const div = new EventDom();
                const spy1 = sinon_1.default.spy();
                const spy2 = sinon_1.default.spy(() => false);
                div.on('click', spy1);
                const node = div.node;
                node.onclick = spy2;
                div.trigger('click');
            ...
            it('should trigger event with EventObject', () => {
                ...
                expect(spy2.callCount).toEqual(1);
                expect(spy3.callCount).toEqual(1);
                div.trigger(new dom_1.Dom.EventObject('click', { namespace: 'ns' }));
                expect(spy1.callCount).toEqual(2);
                expect(spy2.callCount).toEqual(2);
                expect(spy3.callCount).toEqual(1);
            });
            it('should trigger event with EventObject created with native event', () => {
                ...
                evt.preventDefault();
                div.trigger(new dom_1.Dom.EventObject(evt));
                expect(spy1.callCount).toEqual(1);
                expect(spy2.callCount).toEqual(1);
                expect(spy3.callCount).toEqual(1);
            });
            ...
            it('should bind and trigger event on any object', () => {
                const obj = {};
                const spy = sinon_1.default.spy();
                core_1.Core.on(obj, 'foo', spy);
                core_1.Core.trigger('foo', [], obj);
                expect(spy.callCount).toEqual(1);
            });
            ...
            it('should stop propagation when handler return `false`', () => {
                ...
                const spy1 = sinon_1.default.spy();
                const spy2 = sinon_1.default.spy();
                const spy3 = sinon_1.default.spy(() => false);
            ...
            it('should stopImmediatePropagation 1', () => {
                ...
                const container = new EventDom(div);
                const spy1 = sinon_1.default.spy();
                const spy2 = sinon_1.default.spy((e) => {
            ...
            it('should stopImmediatePropagation 2', () => {
                ...
                const evt = document.createEvent('MouseEvents');
                evt.initEvent('click', true, true);
                const node = container.findOne('.one').node;
            ...
            it('should prevent default action', (done) => {
                ...
                (_a = container.findOne('.four')) === null || _a === void 0 ? void 0 : _a.trigger('click');
            });
            it('should do the default action', () => {
                ...
                const spy2 = sinon_1.default.spy();
                const spy3 = sinon_1.default.spy();
                const spy4 = sinon_1.default.spy((e) => {
                    e.stopPropagation();
                });
                container.on('click', spy1);
                container.on('click', '.one', spy2);
        ...
        describe('hooks', () => {
            it('should get event properties on natively-triggered event', (done) => {
                ...
                const evt = document.createEvent('MouseEvents');
                evt.initEvent('click', true, true);
                lk.trigger(new dom_1.Dom.EventObject(evt, { clientX: 10 }));
                lk.remove();
            });
            it('should get event properties added by `addProperty`', (done) => {
                const div = new EventDom().on('click', (e) => {
            ...
            it('shoud add custom event property with `addProperty`', (done) => {
                dom_1.Dom.EventObject.addProperty('testProperty', () => 42);
                const div = new EventDom().on('click', (e) => {
            ...
            it('should call hooks', () => {
                const addHook = sinon_1.default.spy();
                const removeHook = sinon_1.default.spy();
                const setupHook = sinon_1.default.spy();
                const teardownHook = sinon_1.default.spy();
                const handleHook = sinon_1.default.spy();
                const triggerHook = sinon_1.default.spy();
                const preDispatchHook = sinon_1.default.spy();
                const postDispatchHook = sinon_1.default.spy();
                dom_1.Dom.EventHook.register('dblclick', {
                    add: addHook,
                    remove: removeHook,
                    setup: setupHook,
                ...
                const div = new EventDom();
                const spyHandler = sinon_1.default.spy();
                ...
                dom_1.Dom.EventHook.unregister('dblclick');
            });
            it('should not trigger event when `preDispatch` hook return `false`', () => {
                const preDispatchHook = sinon_1.default.spy(() => false);
                dom_1.Dom.EventHook.register('dblclick', {
                    preDispatch: preDispatchHook,
                });
                const div = new EventDom();
            ...
            it('should not trigger event when `trigger` hook return `false`', () => {
                const hook = sinon_1.default.spy(() => false);
                dom_1.Dom.EventHook.register('dblclick', {
                    trigger: hook,
                });
                const div = new EventDom();
            ...
            it('should not prevent default when `preventDefault` hook return `false`', () => {
                const hook = sinon_1.default.spy(() => false);
                dom_1.Dom.EventHook.register('click', {
                    preventDefault: hook,
                });
                const div = new EventDom();
                const spy1 = sinon_1.default.spy();
                ...
                div.on('click', spy3);
                div.trigger('click');
                dom_1.Dom.EventHook.unregister('click');
                expect(spy1.callCount).toEqual(1);
                expect(spy2.callCount).toEqual(1);
                expect(spy3.callCount).toEqual(1);
            });
