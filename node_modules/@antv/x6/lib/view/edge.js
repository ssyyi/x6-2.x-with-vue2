...
class EdgeView extends cell_1.CellView {
    ...
    get sourceBBox() {
        ...
        if (!sourceView) {
            ...
            return new x6_geometry_1.Rectangle(sourceDef.x, sourceDef.y);
        }
        const sourceMagnet = this.sourceMagnet;
        if (sourceView.isEdgeElement(sourceMagnet)) {
            return new x6_geometry_1.Rectangle(this.sourceAnchor.x, this.sourceAnchor.y);
        }
    ...
    get targetBBox() {
        ...
        if (!targetView) {
            ...
            return new x6_geometry_1.Rectangle(targetDef.x, targetDef.y);
        }
        const targetMagnet = this.targetMagnet;
        if (targetView.isEdgeElement(targetMagnet)) {
            return new x6_geometry_1.Rectangle(this.targetAnchor.x, this.targetAnchor.y);
        }
    ...
    renderLabels() {
        ...
        if (container) {
            ...
            container = x6_common_1.Dom.createSvgElement('g');
                ...
                if (previousLabels[index]) {
                    if (pathLength === 2) {
                        ...
                        return (typeof options.propertyValue === 'object' &&
                            x6_common_1.ObjectExt.has(options.propertyValue, 'markup'));
                    }
    ...
    parseLabelStringMarkup(labelMarkup) {
        const children = x6_common_1.Vector.createVectors(labelMarkup);
    ...
    normalizeLabelMarkup(markup) {
        ...
        if (childNodes.length > 1 || childNodes[0].nodeName.toUpperCase() !== 'G') {
            vel = x6_common_1.Vector.create('g').append(fragment);
        }
        else {
            vel = x6_common_1.Vector.create(childNodes[0]);
        }
    ...
    updateLabels() {
        if (this.labelContainer) {
            ...
            for (let i = 0, n = labels.length; i < n; i += 1) {
                ...
                const label = labels[i];
                const attrs = x6_common_1.ObjectExt.merge({}, defaultLabel.attrs, label.attrs);
                this.updateAttrs(elem, attrs, {
                    selectors,
                    rootBBox: label.size ? x6_geometry_1.Rectangle.fromSize(label.size) : undefined,
                });
            }
        }
    }
    ...
    removeRedundantLinearVertices(options = {}) {
        ...
        const polyline = new x6_geometry_1.Polyline(routePoints);
    ...
    getTerminalAnchor(type) {
        switch (type) {
            case 'source':
                return x6_geometry_1.Point.create(this.sourceAnchor);
            case 'target':
                return x6_geometry_1.Point.create(this.targetAnchor);
            default:
                throw new Error(`Unknown terminal type '${type}'`);
        }
    }
    getTerminalConnectionPoint(type) {
        switch (type) {
            case 'source':
                return x6_geometry_1.Point.create(this.sourcePoint);
            case 'target':
                return x6_geometry_1.Point.create(this.targetPoint);
            default:
                throw new Error(`Unknown terminal type '${type}'`);
        }
    }
    ...
    updateConnection(options = {}) {
        ...
        if (options.translateBy &&
            ...
            this.routePoints = new x6_geometry_1.Polyline(this.routePoints).translate(tx, ty).points;
    ...
    findAnchorsOrdered(firstType, firstPoint, secondType, secondPoint) {
        ...
        if (firstView) {
            let firstRef;
            if (firstPoint) {
                firstRef = x6_geometry_1.Point.create(firstPoint);
            }
            else if (secondView) {
                firstRef = secondMagnet;
            }
            else {
                firstRef = x6_geometry_1.Point.create(secondTerminal);
            }
            firstAnchor = this.getAnchor(firstTerminal.anchor, firstView, firstMagnet, firstRef, firstType);
        }
        else {
            firstAnchor = x6_geometry_1.Point.create(firstTerminal);
        }
        if (secondView) {
            const secondRef = x6_geometry_1.Point.create(secondPoint || firstAnchor);
            secondAnchor = this.getAnchor(secondTerminal.anchor, secondView, secondMagnet, secondRef, secondType);
        }
        else {
            secondAnchor = x6_geometry_1.Point.isPointLike(secondTerminal)
                ? x6_geometry_1.Point.create(secondTerminal)
                : new x6_geometry_1.Point();
        }
        return {
    ...
    getAnchor(def, cellView, magnet, ref, terminalType) {
        ...
        if (isEdge) {
            const fn = registry_1.EdgeAnchor.registry.get(name);
            if (typeof fn !== 'function') {
                return registry_1.EdgeAnchor.registry.onNotFound(name);
            }
            anchor = x6_common_1.FunctionExt.call(fn, this, cellView, magnet, ref, config.args || {}, terminalType);
        }
        else {
            const fn = registry_1.NodeAnchor.registry.get(name);
            if (typeof fn !== 'function') {
                return registry_1.NodeAnchor.registry.onNotFound(name);
            }
            anchor = x6_common_1.FunctionExt.call(fn, this, cellView, magnet, ref, config.args || {}, terminalType);
        }
        return anchor ? anchor.round(this.POINT_ROUNDING) : new x6_geometry_1.Point();
    }
    findRoutePoints(vertices = []) {
        const defaultRouter = this.graph.options.connecting.router || registry_1.Router.presets.normal;
        const router = this.cell.getRouter() || defaultRouter;
        let routePoints;
        if (typeof router === 'function') {
            routePoints = x6_common_1.FunctionExt.call(router, this, vertices, {}, this);
        }
        else {
            const name = typeof router === 'string' ? router : router.name;
            const args = typeof router === 'string' ? {} : router.args || {};
            const fn = name ? registry_1.Router.registry.get(name) : registry_1.Router.presets.normal;
            if (typeof fn !== 'function') {
                return registry_1.Router.registry.onNotFound(name);
            }
            routePoints = x6_common_1.FunctionExt.call(fn, this, vertices, args, this);
        }
        return routePoints == null
            ? vertices.map((p) => x6_geometry_1.Point.create(p))
            : routePoints.map((p) => x6_geometry_1.Point.create(p));
    }
    findConnectionPoints(routePoints, sourceAnchor, targetAnchor) {
        ...
        if (sourceView && !sourceView.isEdgeElement(this.sourceMagnet)) {
            ...
            const sourceLine = new x6_geometry_1.Line(sourcePointRef, sourceAnchor);
        ...
        if (targetView && !targetView.isEdgeElement(this.targetMagnet)) {
            ...
            const targetLine = new x6_geometry_1.Line(targetPointRef, targetAnchor);
    ...
    getConnectionPoint(def, view, magnet, line, endType) {
        ...
        const fn = registry_1.ConnectionPoint.registry.get(name);
        if (typeof fn !== 'function') {
            return registry_1.ConnectionPoint.registry.onNotFound(name);
        }
        const connectionPoint = x6_common_1.FunctionExt.call(fn, this, line, view, magnet, args || {}, endType);
    ...
    findPath(routePoints, sourcePoint, targetPoint) {
        ...
        if (name) {
            const method = registry_1.Connector.registry.get(name);
        ...
        const path = x6_common_1.FunctionExt.call(fn, this, sourcePoint, targetPoint, routePoints, Object.assign(Object.assign({}, args), { raw: true }), this);
        return typeof path === 'string' ? x6_geometry_1.Path.parse(path) : path;
    }
    ...
    updateLabelPositions() {
        ...
        for (let i = 0, ii = labels.length; i < ii; i += 1) {
            ...
            const pos = x6_common_1.ObjectExt.merge({}, defaultPosition, labelPosition);
            const matrix = this.getLabelTransformationMatrix(pos);
            labelNode.setAttribute('transform', x6_common_1.Dom.matrixToTransformString(matrix));
        }
        return this;
    }
    ...
    mergeLabelPositionArgs(labelPositionArgs, defaultLabelPositionArgs) {
        ...
        return x6_common_1.ObjectExt.merge({}, defaultLabelPositionArgs, labelPositionArgs);
    }
    // #endregion
    getConnection() {
    ...
    getConnectionPathData() {
        ...
        if (!x6_common_1.ObjectExt.has(cache, 'data')) {
    ...
    getConnectionSubdivisions() {
        ...
        const cache = this.cache.pathCache;
        if (!x6_common_1.ObjectExt.has(cache, 'segmentSubdivisions')) {
    ...
    getConnectionLength() {
        ...
        const cache = this.cache.pathCache;
        if (!x6_common_1.ObjectExt.has(cache, 'length')) {
    ...
    getPointAtRatio(ratio) {
        ...
        if (x6_common_1.NumberExt.isPercentage(ratio)) {
    ...
    getLabelPosition(x, y, p3, p4) {
        ...
        const labelPoint = new x6_geometry_1.Point(x, y);
    ...
    getLabelTransformationMatrix(labelPosition) {
        ...
        if (tangent) {
            ...
            if (isKeepGradient) {
                ...
                if (isEnsureLegibility) {
                    angle = x6_geometry_1.Angle.normalize(((angle + 90) % 180) - 90);
                }
            }
        }
        else {
        ...
        return x6_common_1.Dom.createSVGMatrix()
    ...
    getVertexIndex(x, y) {
        const edge = this.cell;
        const vertices = edge.getVertices();
        const vertexLength = this.getClosestPointLength(new x6_geometry_1.Point(x, y));
        let index = 0;
    ...
    onCustomEvent(e, name, x, y) {
        ...
        const tool = x6_common_1.Dom.findParentByClass(e.target, 'edge-tool', this.container);
        if (tool) {
    ...
    prepareArrowheadDragging(type, options) {
        ...
        const data = {
            ...
            initialTerminal: x6_common_1.ObjectExt.clone(this.cell[type]),
        ...
        const doValidate = (validate) => {
            ...
            const targetPort = terminalType === 'target'
                ...
                : edge
                    ...
                    : null;
            return x6_common_1.FunctionExt.call(validate, this.graph, {
                edge,
                edgeView,
                sourceView,
                targetView,
    ...
    allowConnectToBlank(edge) {
        ...
        return x6_common_1.FunctionExt.call(allowBlank, graph, {
            edge,
            edgeView,
            sourceCell,
            targetCell,
    ...
    validateEdge(edge, type, initialTerminal) {
        ...
        if (validate) {
            return x6_common_1.FunctionExt.call(validate, graph, {
                edge,
                type,
    ...
    snapArrowhead(x, y, data) {
        ...
        const pos = new x6_geometry_1.Point(x, y);
        views.forEach((view) => {
    ...
    notifyConnectionEvent(data, e) {
        ...
        if (changed) {
            ...
            const previousPoint = previousCell || data.isNewEdge
                ? null
                : x6_geometry_1.Point.create(initialTerminal).toJSON();
            ...
            const currentPoint = currentCell
                ? null
                : x6_geometry_1.Point.create(currentTerminal).toJSON();
    ...
    dragLabel(e, x, y) {
        ...
        const label = x6_common_1.ObjectExt.merge({}, originLabel, {
