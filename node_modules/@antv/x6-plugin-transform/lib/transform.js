...
class TransformImpl extends x6_1.View {
    get model() {
    ...
    renderHandles() {
        ...
        x6_1.Dom.attr(knob, 'draggable', 'false');
        const rotate = knob.cloneNode(true);
        x6_1.Dom.addClass(rotate, this.rotateClassName);
        const resizes = Private.POSITIONS.map((pos) => {
            const elem = knob.cloneNode(true);
            x6_1.Dom.addClass(elem, this.resizeClassName);
            x6_1.Dom.attr(elem, 'data-position', pos);
            return elem;
        });
        this.empty();
        x6_1.Dom.append(this.container, [...resizes, rotate]);
    }
    render() {
        this.renderHandles();
        if (this.view) {
        ...
        x6_1.Dom.addClass(this.container, this.containerClassName);
        x6_1.Dom.toggleClass(this.container, 'no-orth-resize', this.options.preserveAspectRatio || !this.options.orthogonalResizing);
        x6_1.Dom.toggleClass(this.container, 'no-resize', !this.options.resizable);
        x6_1.Dom.toggleClass(this.container, 'no-rotate', !this.options.rotatable);
    ...
    update() {
        ...
        bbox.width *= ctm.a;
        bbox.height *= ctm.d;
        const angle = x6_1.Angle.normalize(this.node.getAngle());
    ...
    updateResizerDirections() {
        ...
        if (shift !== this.prevShift) {
            ...
            resizes.forEach((resize, index) => {
                x6_1.Dom.removeClass(resize, Private.DIRECTIONS.map((dir) => className(dir)).join(' '));
                x6_1.Dom.addClass(resize, className(directions[index]));
            });
    ...
    startResizing(evt) {
        ...
        const dir = x6_1.Dom.attr(evt.target, 'data-position');
    ...
    startRotating(evt) {
        ...
        this.setEventData(evt, {
            center,
            action: 'rotating',
            angle: x6_1.Angle.normalize(this.node.getAngle()),
            start: x6_1.Point.create(client).theta(center),
        });
        this.startAction(evt);
    }
    onMouseMove(evt) {
        ...
        if (data.action) {
            ...
            if (restrict === true || typeof restrict === 'number') {
                ...
                clientX = x6_1.NumberExt.clamp(clientX, rect.left + fix, rect.right - fix);
                clientY = x6_1.NumberExt.clamp(clientY, rect.top + fix, rect.bottom - fix);
            }
            ...
            if (data.action === 'resizing') {
                ...
                const requestedSize = x6_1.Point.create(pos)
                ...
                width = x6_1.GeometryUtil.snapToGrid(width, gridSize);
                height = x6_1.GeometryUtil.snapToGrid(height, gridSize);
                ...
                const theta = data.start - x6_1.Point.create(pos).theta(data.center);
                let target = data.angle + theta;
                if (options.rotateGrid) {
                    target = x6_1.GeometryUtil.snapToGrid(target, options.rotateGrid);
                }
                target = x6_1.Angle.normalize(target);
    ...
    startHandle(handle) {
        ...
        x6_1.Dom.addClass(this.container, `${this.containerClassName}-active`);
        if (handle) {
            x6_1.Dom.addClass(handle, `${this.containerClassName}-active-handle`);
            const pos = handle.getAttribute('data-position');
            if (pos) {
                const dir = Private.DIRECTIONS[Private.POSITIONS.indexOf(pos)];
                x6_1.Dom.addClass(this.container, `${this.containerClassName}-cursor-${dir}`);
            }
        }
    }
    stopHandle() {
        x6_1.Dom.removeClass(this.container, `${this.containerClassName}-active`);
        if (this.handle) {
            x6_1.Dom.removeClass(this.handle, `${this.containerClassName}-active-handle`);
            const pos = this.handle.getAttribute('data-position');
            if (pos) {
                const dir = Private.DIRECTIONS[Private.POSITIONS.indexOf(pos)];
                x6_1.Dom.removeClass(this.container, `${this.containerClassName}-cursor-${dir}`);
            }
            this.handle = null;
        }
    }
